...
local function showTitleAnimation()
    ...
    pcall(function()
        local tweenToCyan = TweenService:Create(titleLabel, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
...
local function continueStartup()
    ...
    task.spawn(function()
        while topText.Parent do
            pcall(function() TweenService:Create(topText, TweenInfo.new(0.5), {TextTransparency = 0.7}):Play() end)
            task.wait(0.5)
            pcall(function() TweenService:Create(topText, TweenInfo.new(0.5), {TextTransparency = 0}):Play() end)
            task.wait(0.5)
        end
    end)
...
function loadMainScript()
    ...
    local function showKeyUI(callback)
        ...
        container.Size = UDim2.new(0, 280, 0, 160)
        container.Position = UDim2.new(0.5, -140, 0.5, -80)
        ...
        stroke.Thickness = 2
        stroke.Color = Color3.fromRGB(0, 200, 220)
        ...
        confirmBtn.Position = UDim2.new(0.5, -50, 0, 108)
        confirmBtn.BackgroundColor3 = Color3.fromRGB(0, 200, 220)
        confirmBtn.Text = "确认"
    ...
    local function triggerEasterEgg()
        ...
        eggGui.Parent = LocalPlayer:WaitForChild("PlayerGui")
    ...
    local function onProfileClick(name)
        ...
        if clickedCount >= totalProfileCount then
            showKeyUI(function(success)
                if success then triggerEasterEgg() end
            end)
        end
    end
    ...
    local sectionESP = tabCommon:Section("透视", {Y = "89197120299250", F = "89197120299250"}, true)
