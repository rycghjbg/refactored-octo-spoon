...
frame.BackgroundColor3 = Color3.fromRGB(30, 0, 60)   -- 深科技紫
...
local function showStartupEffect()
    ...
    task.delay(3, function()
        ...
        effectGui:Destroy()
        loadMainScript()   -- 特效结束后加载你的完整功能脚本
    end)
end
...
-- ==================== 4. 原功能脚本（完全没改动，放在函数里延迟加载） ====================
function loadMainScript()
    -- 以下是你原来的全部代码，从 local adminList 一直到最后，原封不动
    local adminList = {
        "zxc110819",
        "NOOOPLSDONTletme444",
        "aa1360051",
    ...
    if isInList(adminList, LocalPlayer.Name) then
        windowTitle = "测试人员"
        titleColor = Color3.fromRGB(255, 215, 0)   -- 金色
        isRainbowTitle = false
        userRoleName = "测试人员"                  -- 用于权限Tab和功能名称
    elseif isInList(authorList, LocalPlayer.Name) then
        windowTitle = "款脚本作者"
        titleColor = Color3.fromRGB(0, 255, 255)   -- 青色
        isRainbowTitle = true
        userRoleName = "作者"                      -- 作者权限显示“作者”
    else
        windowTitle = "脚本使用者"
        titleColor = Color3.fromRGB(0, 0, 0)       -- 黑色
        isRainbowTitle = false
        userRoleName = nil
    end
    ...
    -- ==================== 动态权限系统 ====================
    if IsAdminOrAuthor() and userRoleName then   -- 确保有角色名
